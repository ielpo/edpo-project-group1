...
# --- Pin Configuration ---
LED_EN = Pin(19, mode=Pin.OPEN_DRAIN, pull=0, value=False)
IRQ_IN = Pin(20, mode=Pin.IN)
SDA_PIN = Pin(8)
SCL_PIN = Pin(9)
...
class TCS34725:
    ...
    def _read_u8(self, address: int) -> int:
        ...
        memaddr = (address | _COMMAND_BIT) & 0xFF
        buffer = bytearray(1)
        self._device.readfrom_mem_into(self._address, memaddr, buffer)
        return buffer[0]
    ...
    def _write_u8(self, address: int, val: int):
        ...
        buffer = bytearray(1)
        buffer[0] = val & 0xFF
        self._device.writeto_mem(self._address, memaddr, buffer)
...
def start_server(sensor: TCS34725, ip: str):
    ...
    while True:
        ...
        try:
            ...
            if method == "GET" and path == "/color":
                ...
                print(f"Read {value} from sensor")
...
def alive_blink(t: Timer):
    global PICO_LED_STATE
    if PICO_LED_STATE:
        PICO_LED.value(False)
        PICO_LED_STATE = False
        t.init(mode=Timer.ONE_SHOT, period=2000, callback=alive_blink)
    else:
        PICO_LED.value(True)
        PICO_LED_STATE = True
        t.init(mode=Timer.ONE_SHOT, period=80, callback=alive_blink)
